...
class LLMMemExpander:
    ...
    def _build_multiple_candidates_prompt(self, context: Dict[str, Any]) -> str:
        """构建LLM扩展提示"""
        # 加载 Mhealth 架构信息
        mhealth_architectures = load_mhealth_architectures('/root/tinyml/arch_files/Mhealth.json')
        tier_1_models = [arch for arch in mhealth_architectures if arch['tier'] == 'Tier 1']
        tier_2_models = [arch for arch in mhealth_architectures if arch['tier'] == 'Tier 2']

        # 详细格式化模型信息 - 包含完整的架构配置
        tier_1_info = "\n\n".join(
            [
                f"""- Tier 1 Model {i+1}:
                    Memory: {arch['total_memory_MB']:.2f} MB, Latency: {arch['latency']:.2f} ms
                    Configuration:
                    {json.dumps(arch['configuration'], indent=4, ensure_ascii=False)}"""
                                for i, arch in enumerate(tier_1_models)
            ]
        )
        ...
        tier_2_info = "\n\n".join(
            [
                f"""- Tier 2 Model {i+1}:
                    Quantized Memory: {arch['quantized_memory_MB']:.2f} MB (Original: {arch['peak_memory_MB']:.2f} MB)
                    Latency: {arch['latency']:.2f} ms, Accuracy Drop: {arch.get('accuracy_drop', 'N/A')}%
                    Configuration:
                    {json.dumps(arch['configuration'], indent=4, ensure_ascii=False)}"""
                                for i, arch in enumerate(tier_2_models)
            ]
        )
        ...
        prompt = """
            ...
            **Important Notes:**
            ...
            - When the memory limit is very strict, if you don't want to use quantization, you can consider using SeDpConv (without setting the has_se True)
            ...
            **Return format example:**
            {{
                "candidates": [
                    ...
                    tier_1_info=tier_1_info,
                    tier_2_info=tier_2_info,
    ...
    def _generate_improvement_suggestions(self, failure_reasons: Dict, valid_candidates: List[Dict]) -> str:
        ...
        if 'memory_constraint' in failure_reasons:
            ...
            suggestions.append("   - SeDpConv is the lightest block")
            suggestions.append("   - The memory usage of MBConv should be the highest, while the memory of DWSeqConv should be the second highest.")
