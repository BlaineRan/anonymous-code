...
class MCTSArchitectureSearcher:
    ...
    def __init__(self, llm_config: Dict[str, Any], search_space: Dict[str, Any], 
        ...
        self.llm_expander = SFSPredictorExpander(llm_config, search_space, self.dataset_info)
        # 需要在初始化完成后设置图结构引用
    ...
    def search(self, total_iterations: int = 100) -> Dict[str, Any]:
        """执行完整的MCTS架构搜索"""
        print("🚀 开始MCTS架构搜索")
    ...
    def _mcts_iteration(self, dataset_name: str, dataloader, save_dir: str, iteration: int) -> Optional[ArchitectureNode]:
        ...
        # 1. 选择父节点进行扩展
        parent_node = self.search_graph.select_parent_for_sfsexpansion()
        print(f"📍 选择父节点进行扩展，访问次数: {parent_node.visits}")

        # === 新增：选择量化方向 ===
        selected_direction = self.search_graph.select_direction_uct(parent_node)
        print(f"🎯 选择的量化方向: {selected_direction}")
        
        # 2. 扩展节点（传入选定的方向）
        print(f"parent_node.node_id: {parent_node.node_id}\nparent_node.candidate: {parent_node.candidate}")
        new_candidate = self._expand_node(parent_node, dataset_name, selected_direction)
        ...
        new_node.direction = selected_direction  # 记录使用的方向
        ...
        # === 新增：反向传播 ===
        self._backpropagate(parent_node, new_node, reward, selected_direction)
    ...
    def _backpropagate(self, parent_node: ArchitectureNode, child_node: ArchitectureNode, 
                   reward: float, direction: str):
        """反向传播：更新路径上所有节点的统计信息"""
        current_node = parent_node
        
        # 更新父节点的方向统计
        self.search_graph.update_direction_stats(current_node, direction, reward)
        
        # 可选：继续向上传播到祖先节点（简化版本只更新直接父节点）
        print(f"🔄 反向传播: 方向 '{direction}' 得分 {reward:.3f} 传播到父节点")
    
    def _expand_node(self, node: ArchitectureNode, dataset_name: str, direction: str) -> Optional[CandidateModel]:
        """扩展节点，生成新的架构"""
        # 获取Pareto前沿反馈
        ...
        # 确保生成的候选模型使用指定的量化方向
        if new_candidate:
            new_candidate.config['quant_mode'] = direction
...
def main():
    ...
    # 2. 选择要搜索的数据集
    # 可以选择单个数据集进行快速测试
    dataset_names = ['UTD-MHAD']  # 或者 ['USCHAD', 'WISDM'] 用于多数据集
    ...
    try:
        ...
        results = searcher.search(total_iterations=20)  # 先用小数量测试
        ...
        print(f"\n✅ 详细结果已保存到: /root/tinyml/weights/mcts_search/")
