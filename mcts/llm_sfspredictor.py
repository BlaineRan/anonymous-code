...
class SFSPredictorExpander:
    ...
    def set_dataset_info(self, dataset_info: Dict[str, Any]):
        ...
        # 不应该把predictor的初始化放在这里，它有个参数dataset_name

        # 重新初始化predictor以适应新的数据集
        # self._initialize_predictor()
    ...
    def expand_from_parent(self, parent_node: ArchitectureNode, dataset_name: str, 
                          ...
                          global_failures: List[Dict] = None,
                          direction: str = "none") -> Optional[CandidateModel]:  # 新增direction参数，默认值为"none"
        """基于 父节点 和 反馈生成新的架构"""
        ...
        # === 新增：记录使用的量化方向 ===
        print(f"🎯 使用量化方向: {direction}")
        session_failures.append({
            'quantization_direction': direction,
            'direction_applied': False  # 初始标记为未应用
        })
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                try:
                    ...
                    # === 关键：确保选择的候选使用指定的量化方向 ===
                    best_candidate.config['quant_mode'] = direction
                    best_candidate.metadata['quantization_mode'] = direction

                    # 标记方向已成功应用
                    session_failures[-1]['direction_applied'] = True
                    ...
                    # 构建详细的失败反馈
                    validation_feedback = self._build_quality_failure_feedback(failure_report, attempt, direction)
                    # 记录到session_failures
                    session_failures.append({
                        'attempt': attempt + 1,
    ...
    def _build_multiple_candidates_prompt(self, context: Dict[str, Any]) -> str:
        ...
        # 读取JSON文件
        with open('/root/tinyml/arch_files/model_utd-mhad.json', 'r') as f:
            data = json.load(f)

        # 提取架构信息
        arch_info = []
        for model in data['model_comparisons']:
            info = f"{model['model_description']}: Memory={model['peak_memory_mb']}MB Latency={model['inference_latency_ms']}ms **Accuracy={model['accuracy']}%**"
