...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Dynamically generate architecture models and perform training and quantization")
    parser.add_argument(
        "--dataset_name", 
        type=str, 
        required=True, 
        help="Specify the dataset name to use, e.g., UTD-MHAD or other datasets"
    )
...
def train_qat_model(model, dataloader, device, save_path, logger, epochs=100):
    """Train QAT model - Train untrained model from scratch"""
    try:
...
def test_model_worker(config, description, dataset_name, base_save_dir, gpu_id, result_queue, logger):
    ...
    try:
        ...
        # 3. QAT Quantization Aware Training - Use brand new untrained model
        ...
        # Create new untrained model for QAT
        candidate = CandidateModel(config=config)
...
if __name__ == "__main__":
    # Parse command line arguments
    args = parse_args()
    # Get dataset_name and other configurations from command line arguments
    ...
    try:
        ...
        num_configs = 4000  # Generate 10000 different architectures
