...
import time
from Proxyless.rznas_proxies import RZNASProxies
import torch
...
class LLMRZNAS:
    ...
    def expand_from_parent(self, parent_node: ArchitectureNode, dataset_name: str, 
        ...
        # 🔥 修正：如果所有尝试都失败，使用 proxy_score 进行后备选择
        ...
        if all_valid_candidates:
            ...
            if unique_valid_candidates:
                ...
                print(f"{'=' * 20}\n🎯 后备选择最佳候选:\n{'=' * 20}\n")
                print(f"   proxy分数: {best_candidate_info['proxy_score']:.3f}")
    ...
    def _validate_candidate(self, candidate: CandidateModel, dataset_name: str) -> tuple:
        ...
        estimated_total_size_status = f"Estimated Total Size: {effective_memory:.2f}MB"
        ...
        if effective_memory > 4 * effective_limit:
            ...
            print(f"✅ 内存约束检查通过: {effective_memory:.2f}MB")
    ...
    def _build_multiple_candidates_prompt(self, context: Dict[str, Any]) -> str:
        ...
        if context.get('parent_architecture') is not None and 'parent_architecture' in context:
            ...
            - MCTS Score: {parent['performance']['mcts_stats']['score']:.3f}
            - Zero-Cost Proxy Score: {parent.get('proxy_score', 'N/A')}  # 新增
            - Visits: {parent['performance']['mcts_stats']['visits']}
            - Evaluated: {parent['performance']['mcts_stats']['is_evaluated']}"""
        ...
        expected_memory = str(constraints['max_peak_memory'] * 0.75)  # 期望内存为0.75倍
        ...
        prompt = """
            ...
            **Memory-Aware Architecture Strategy:**
            ...
            - Target {expected_memory}-{max_peak_memory} MB models for best accuracy
            ...
            **Important Rule**
            ...
            - Modify: Change parameters of an existing stage or block (e.g., kernel_size, expansion, channels, has_se, etc.)
    ...
    def _review_and_select_candidate(self, candidates: List[CandidateModel], 
        ...
        proxy_evaluator = RZNASProxies(self.search_space, device=device, dataset_name=dataset_name)
        ...
        for i, candidate in enumerate(candidates, 1):
            try:
                ...
                try:
                    ...
                    print(f"   - grad_norm 效率: {raw_scores['grad_norm']:.3f}")
                    ...
                    #     print(f"   - 量化友好: {raw_scores['quant_friendliness']:.3f}")
                    
                except Exception as e:
                    print(f"⚠️ Zero-Cost代理方法评估失败: {e}")
                    proxy_score = 0.1  # 给一个较低的默认分数
                    raw_scores = {'grad_norm': 0, 'zen': 0, 'synflow': 0, 'zico': 0, 'grasp': 0}
                ...
                print(f"✅ 候选{i}通过验证， 期望达成: {meets_expectation} 代理分数: {proxy_score:.4f}")
        ...
        print(f"   🧠 Zero-Cost代理分数: {selected['proxy_score']:.4f}")
        print(f"   内存期望达成: {selected['meets_expectation']}")
        print(f"   详细分数: {selected['raw_proxy_scores']}")
        
        # 打印所有候选的比较
        print(f"\n📊 所有候选比较:")
        for i, cand in enumerate(valid_candidates, 1):
            status = "✅ 选中" if i == 1 else "  "
            print(f"{status} 候选{i}: {cand['effective_memory']:.1f}MB (代理分数: {cand['proxy_score']:.4f})")
